...
public class OrderService {
    ...
    public OrderResponse createOrder(OrderRequest orderRequest) {
        ...
        List<OrderItemResponse> orderItems = orderRequest.getItems().stream().map(orderItemRequest -> {
            try {
                ...
                MenuItemResponse menuItem = webClient.get()
                        .uri("http://localhost:8084/api/menu-items/{id}", orderItemRequest.getMenuItemId())
                        .retrieve()
                        .bodyToMono(MenuItemResponse.class)
                        .block();

                // Calculate the total price for the item (price * quantity)
                BigDecimal itemTotalPrice = menuItem.getPrice().multiply(BigDecimal.valueOf(orderItemRequest.getQuantity()));
                ...
                throw new MenuItemNotFoundException("Menu item not found: " + orderItemRequest.getMenuItemId());
            }
        }).collect(Collectors.toList());

        // Calculate the total price for the entire order
        BigDecimal totalOrderPrice = orderItems.stream()
                .map(orderItemResponse -> orderItemResponse.getPrice().multiply(BigDecimal.valueOf(orderItemResponse.getQuantity())))
        ...
        // Return the OrderResponse containing all items, total price, and the generated ID
